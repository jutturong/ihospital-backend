...
import { AssetModel } from '../../models/budget/asset';
...
router.get('/listCom', (req, res, next) => {
  ...
  model.listCom(db,department,fiscal_year)
    .then((results: any) => {
...
router.get('/listMed', (req, res, next) => {
  ...
  model.listMed(db,department, fiscal_year)
    .then((results: any) => {
      res.send({ ok: true, rows: results });
    })
    .catch(error => {
...
router.get('/listGen', (req, res, next) => {
  ...
  model.listGen(db,department,fiscal_year)
    .then((results: any) => {
      res.send({ ok: true, rows: results });
    })
    .catch(error => {
...
router.get('/listOth', (req, res, next) => {
  ...
  model.listOth(db,department,fiscal_year)
    .then((results: any) => {
      res.send({ ok: true, rows: results });
    })
    .catch(error => {
