...
export class Payback {
  ...
  public tableName = 'bloodbank_payback';
  public primaryKey = 'payback_id';
  ...
  listByFk(knex: Knex, req_id: number, limit: number = 100, offset: number = 0) { 
    return knex.select('c.*','p.payback_id', 'p.payer_code', 'p.payer_name', 'p.pay_date', 'p.pay_time', 'recipient_name', 'return_date', 'return_time', 'p.tr', 'p.sell', 'p.share', )
      .from('bloodbank_crossmatch AS c')
      .leftJoin('bloodbank_payback AS p', 'c.crossmatch_id', 'p.crossmatch_id')
  ...
  listByPk(knex: Knex, crossmatch_id: any, limit: number = 100, offset: number = 0) { 
    ...
    return knex.select("c.*", "r.*", "g.abo_type", "t.code2", "i.ref", "i.item_code", "i.unit", 'i.ml', "t.name AS lab_name", "p.title", "p.name", "p.surname", patient_name, "d1.desc AS dx_name1", "d2.desc AS dx_name2", "d3.desc AS dx_name3", "p.ward_name", "s.specimen_name", "f.request_name", input_name, collected_name, verify_name, dr_name, full_name)
      .from('bloodbank_crossmatch AS c')
  ...
  update(knex: Knex, payback_id: number, datas: any) {
